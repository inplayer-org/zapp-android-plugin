...
import com.inplayer.zapp.kit.mocks.MockPluginConfigurator
 ...
 * removed
 */
class InPlayerLoginPlugin : LoginContract, PluginScreen, HookScreen {
    ...
    companion object {
        var INSTANCE: InPlayerLoginPlugin? = null

        fun init(context: Context, merchantUUID: String, referrer: String): InPlayerLoginPlugin {
            if (INSTANCE == null) {
                INSTANCE = InPlayerLoginPlugin()
                InPlayer.initialize(
                    InPlayer.Configuration.Builder(
                        context,
                        merchantUUID
                    ) //Optional
                        .withReferrer(referrer) //Optional -Default is set to Production
                        .withEnvironment(InPlayer.EnvironmentType.STAGING)
                        .build()
                )
            }
            return INSTANCE as InPlayerLoginPlugin
        }
...
//        TODO("Handle purchasable/locked items") acces.granted should update boolean here from  inplayer notification
        return isItemLocked
    }
